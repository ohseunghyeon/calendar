...
* 서버
    ...
    2. in-memery db 라이브러리를 찾아보고 마땅한 게 없으면 메모리에 데이터를 저장하자 (redis 사용)
...
* 클라이언트
    ...
    3. 데이터 관리를 어떻게 할까
        ...
        * global variable로 월(default)/주 의 값, 현재 보고 있는 년월주, 일정 데이터를 두자
        * 일정 데이터는 특정 flag를 두고 해당 flag가 변경됐는데 true일 때 가져오게 호출하자
        * 이 flag는 
            * 생성, 수정, 삭제 AJAX 호출 시 true로 변경하게 하자 GET 시에는 false로 변경
            * 일정의 변경 시에도 true로 변경하게 하자
        * 일정 데이터 호출은 현재 보고 있는 year, month, week을 바탕으로 start, end unix timestamp를 생성하여 body에 포함

    4. CSS는? material-ui를 기본적으로 써보고, 나머지는 css 수작업
